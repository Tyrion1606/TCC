...
A alimentação foi distribuída entre as duas camadas da plataforma. Na montagem, a Raspberry Pi 3 Model B+ foi alimentada por um powerbank Peining WUP-939, com capacidade nominal de 20~Ah a 3,7~V, correspondente a 74~Wh, e saída USB de 5~V e até 2,1~A. A STM32 recebeu alimentação pela conexão USB utilizada para a comunicação com a Raspberry Pi, enquanto o ADS1256 e a tela TFT auxiliar foram alimentados pelo conjunto de alimentação associado à STM32.

Para estimar o consumo do núcleo de aquisição, foram adotados os seguintes valores nominais aproximados de:

\begin{itemize}
    \item 30~mA em 3,3~V para a Black Pill;
    \item 38~mW para o ADS1256;
    \item 50~mA em 3,3~V para a tela TFT auxiliar;
    \item 170~$\mu$A em 3,3~V para cada um dos dois módulos AD8232;
    \item 4~mA em 5~V para o módulo de PPG.
\end{itemize}
...
\begin{equation}
\begin{split}
P_{\text{aquisição}} \approx {}&
(3{,}3 \times 0{,}030)
+ 0{,}038
+ (3{,}3 \times 0{,}050)\\
&+2(3{,}3 \times 170\times10^{-6})
+(5 \times 0{,}004)
\approx 0{,}32~\mathrm{W}.
\end{split}
\end{equation}

Como esse consumo é inferior a 1~W e representa aproximadamente 6\% da carga total estimada, sua influência sobre a autonomia é comparativamente irrelevante diante da Raspberry Pi e da tela principal.

A Raspberry Pi 3 Model B+ apresenta consumo ativo típico próximo de 500~mA, correspondente a aproximadamente 2,5~W em 5~V. A tela principal de 7~polegadas consome aproximadamente 490~mA em 5~V, ou 2,45~W. Portanto,
...
Considerando a energia nominal do powerbank, a autonomia estimada é

\begin{equation}
t_{\mathrm{nominal}}
=
\frac{E_{\mathrm{nominal}}}{P_{\mathrm{total}}}
=
\frac{74~\mathrm{Wh}}{5{,}27~\mathrm{W}}
\approx
14{,}0~\mathrm{h}.
\end{equation}

Esse resultado deve ser entendido como uma estimativa nominal, pois a autonomia efetiva pode variar conforme o consumo médio do conjunto e as perdas de conversão de energia.

O modelo WUP-939 não constitui requisito da arquitetura. Em aplicações futuras, ele pode ser substituído por um conjunto de baterias associado a um controlador de carga, desde que a solução forneça tensão estável e corrente suficiente para o funcionamento do sistema. Quando a portabilidade não for necessária, também pode ser utilizada uma fonte de alimentação USB compatível.

A Figura~\ref{fig:prototipo_final} apresenta a configuração física final da plataforma, incluindo o núcleo de aquisição em placa perfurada, a Raspberry Pi, as duas telas, o powerbank e as conexões entre as camadas. Os módulos de condicionamento de ECG e PPG são conectados às entradas do núcleo de aquisição e não aparecem nessa fotografia.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.88\textwidth]{figuras/prototipo_final_plataforma_portatil.jpg}
    \caption[Configuração física final da plataforma]{Configuração física final da plataforma. À esquerda, apresenta-se a tela sensível ao toque de 7~polegadas; na parte superior direita, o núcleo de aquisição em placa perfurada, composto pela STM32 Black Pill, pelo ADS1256 e pela tela TFT auxiliar; na parte inferior direita, a Raspberry Pi 3 Model B+; e, na parte inferior esquerda, o powerbank Peining WUP-939 utilizado para alimentação portátil.}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{figuras/print_aquisicao.png}
...
As dimensões externas do conjunto foram de 20~cm por 15~cm por 5m. A massa total, considerando a caixa, o conjunto de aquisição composto pelo STM32 + Display + ADS1256, o PowerBank e a Raspberry pi, foi de aproximadamente 1~Kg.
...
O custo de reposição da plataforma foi estimado a partir de preços de componentes novos ou equivalentes observados em lojas on-line, por exemplo, no Mercado Livre, em julho de 2026. Os valores da Tabela~\ref{tab:custo_plataforma} são apenas referências aproximadas e podem variar conforme vendedor, disponibilidade e frete.

\begin{table}[!htbp]
    \centering
    \caption{Estimativa de custo de reposição dos componentes da plataforma, com base em preços de lojas on-line em julho de 2026.}
    ...
    \begin{tabular}{p{7.0cm} c c}
        ...
        \hline
        \textbf{Custo total aproximado de reposição} &
        -- &
        \textbf{R\$~1.380,00} \\
        \hline
    \end{tabular}
...
A maior parcela da estimativa está na Raspberry Pi e na tela principal, que juntas representam aproximadamente 52\% do total. Em comparação, a STM32, o ADS1256, a tela auxiliar e os módulos de condicionamento possuem custo reduzido; os elementos de conexão e montagem têm participação ainda menor. Assim, o valor apresentado caracteriza a reposição aproximada do protótipo acadêmico, e não o custo de uma versão comercial.
